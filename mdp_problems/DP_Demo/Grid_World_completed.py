...
def bellman_update(env, V, pi, s, gamma):
    ...
    for action in env.A:
        ...
        # find q_value of taking state-action pair
        q_value = 0
        for s_, (r, p) in enumerate(transition):
            q_value += p * (r + gamma * V[s_])
        summation += pi[s][action] * q_value
    V[s] = summation
    ### END CODE HERE ###
...
def q_greedify_policy(env, V, pi, s, gamma):
    ...
    ### START CODE HERE ###
    greedy_action = env.A[0]
    biggest_q_val = float("-inf")
    for action in env.A:
        transition = env.transitions(s, action)
        q_val = 0
        for s_, (r, p) in enumerate(transition):
            q_val += p * (r + gamma * V[s_])
        # update greedy action with the biggest q_val
        if q_val > biggest_q_val:
            greedy_action = action
            biggest_q_val = q_val
    
    pi[s] = 0
    pi[s][greedy_action] = 1
    ### END CODE HERE ###
...
def bellman_optimality_update(env, V, s, gamma):
    ...
    ### START CODE HERE ###
    biggest_qval = float('-inf')
    for action in env.A:
        transition = env.transitions(s, action)
        qval = 0
        for s_, (r, p) in enumerate(transition):
            qval += p * (r + gamma * V[s_])
        if qval > biggest_qval:
            biggest_qval = qval
    V[s] = biggest_qval
    ### END CODE HERE ###
