...
# WOFPlayer class definition (part A)
class WOFPlayer:
    # Constructor
    ...
    # casting the class into string
    def __str__(self):
        return "{} (${})".format(self.name, self.prizeMoney)


# WOFHumanPlayer class definition (part B)
class WOFHumanPlayer(WOFPlayer):
    # Doing a move in the game
...
# WOFComputerPlayer class definition (part C)
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, name, difficulty):
        ...
        self.prizeMoney = 0
        self.prizes = []
        self.difficulty = difficulty # range from 1 to 10

    # Decide good or bad move
    # True - good move, False - bad move
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        # possible choices for the move
        possible_letters = self.getPossibleLetters(guessed)

        # do nothing if no choice
        if possible_letters == []:
            return 'pass'

        # depending on good/bad move
        good_move = self.smartCoinFlip()
        if good_move:
            # traverse from back to front in SORTED_FREQUENCIES
            for i in range(len(self.SORTED_FREQUENCIES)):
                if self.SORTED_FREQUENCIES[len(self.SORTED_FREQUENCIES) - i - 1] in possible_letters:
                    return self.SORTED_FREQUENCIES[len(self.SORTED_FREQUENCIES) - i - 1]
        else:
